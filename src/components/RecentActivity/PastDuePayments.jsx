...
// FIX: Added filter to exclude bills with category 'Bill Prep'.
...
import {
    ...
    IconAlertTriangle, // Icon for Past Due title and items
    IconHome,
    IconBolt,
    IconCreditCard,
    IconCar,
...
import isSameOrBefore from 'dayjs/plugin/isSameOrBefore'; // Import isSameOrBefore
...
const getCategoryIcon = (category) => {
    ...
    // Exclude Bill Prep icon here if needed, or handle in filter
...
const PastDuePayments = ({ style }) => {
  ...
  return (
    <Spin spinning={loading} size="small">
      <Card
        // Keep height 100% for consistency if added previously
        style={{ ...style, height: '100%' }}
        title={
            <Title level={4} style={{ margin: 0, display: 'flex', alignItems: 'center' }}>
                {/* Using a more distinct past due icon */}
                <IconAlertOctagonFilled size={24} style={{ marginRight: 'var(--space-8)', color: 'var(--danger-500)' }} />
                Past Due Payments {/* Show count */}
            </Title>
        }
        extra={collapseButton}
        // Adjust padding if list exists
        styles={{ body: { padding: pastDuePaymentsInView.length > 0 ? '0 20px 16px 20px' : '20px' } }}
      >
        {!isCollapsed && (
            <>
                {pastDuePaymentsInView.length === 0 && !loading ? (
                    <Empty description="No past due payments" image={Empty.PRESENTED_IMAGE_SIMPLE} style={{ padding: 'var(--space-32) 0' }}/>
                ) : (
                <>
                    <List
                    ...
                    renderItem={(item) => {
                        ...
                        return (
                            <List.Item style={{ padding: '12px 0', borderBottom: '1px solid var(--neutral-200)' }}>
                                <Space align="center" style={{ width: '100%', justifyContent: 'space-between' }}>
                                    ...
                                    <Text strong style={{ fontSize: '0.875rem', color: 'var(--danger-700)' }}> {/* Darker red for amount */}
