...
const EnhancedBillRow = ({
  ...
  const triggerHaptic = React.useCallback((type = 'light') => {
...
const CombinedBillsOverview = ({ style }) => {
  ...
  return (
    <>
      <Card style={{ ...style, borderRadius: '20px', boxShadow: 'none' }}>
          ...
          >
            <MonthlyProgressSummary
              ...
              totalBillsInDisplayedMonth={
                billsDueInDisplayedMonth.length
              }
              paidBillsInDisplayedMonth={
                billsDueInDisplayedMonth.filter((b) => b.isPaid).length
              }
              ...
              percentAmountPaid={
                billsDueInDisplayedMonth.length > 0
                  ? Math.round(
                      (billsDueInDisplayedMonth
                        .filter((b) => b.isPaid)
          ...
          >
            {billsDueInDisplayedMonth.length > 0 && paidVisibleCount > 0 && (
              <Button
                type="text"
                icon={
                  showPaidBills ? <IconEyeOff size={16} /> : <IconEye size={16} />
                }
                onClick={(e) => {
                  e.stopPropagation();
                  setShowPaidBills((prev) => !prev);
