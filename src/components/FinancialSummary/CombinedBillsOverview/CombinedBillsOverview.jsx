...
const CombinedBillsOverview = ({ style }) => {
    ...
    const columns = [
        { title: 'Status', dataIndex: 'isPaid', key: 'statusCheckbox', width: 35, align: 'center', render: (isPaid, record) => (<Tooltip title={isPaid ? "Mark as Unpaid" : "Mark as Paid"}><Checkbox className={`status-checkbox small-checkbox ${isPaid ? 'checked' : ''}`} checked={isPaid} onChange={() => handleTogglePaid(record)} /></Tooltip>) },
        { title: 'Name', dataIndex: 'name', key: 'name', width: 80, align: 'left', sorter: (a, b) => a.name.localeCompare(b.name), render: (text) => (<div style={{ textAlign: 'left' }}><Text strong>{text}</Text></div>) },
